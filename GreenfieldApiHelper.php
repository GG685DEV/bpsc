...
class GreenfieldApiHelper {
	...
	public function __construct(?int $orderId = null) {
		if ($config = self::getConfig($orderId)) {
			$this->url = $config['url'];
	...
	public static function shouldUseSecondary(?int $orderId = null): bool {
		$userId = 0;
		...
		if (in_array('blacklisted', (array) $user->roles, true)) {
			return false;
		}
	...
	public static function getConfig(?int $orderId = null): array {
		// todo: perf: maybe add caching
				$suffix = self::shouldUseSecondary($orderId) ? '_sec' : '';

		$url = get_option('btcpay_gf_url' . $suffix);
				$key = get_option('btcpay_gf_api_key' . $suffix);
		if ($url && $key) {
			return [
				'url' => rtrim($url, '/'),
				'api_key' => $key,
				'store_id' => get_option('btcpay_gf_store_id' . $suffix, null),
				'webhook' => get_option('btcpay_gf_webhook' . $suffix, null)
			];
		}
		else {
			return [];
